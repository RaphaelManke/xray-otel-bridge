...
const xrayListener = require("./xray-listener");
const telemetryDispatcher = require("./telemetry-dispatcher");
const xrayDispatcher = require("./xray-dispatcher");
...
(async function main() {
  ...
  // Step 2a - Start the local http listener which will receive data from Telemetry API
  ...
  // Step 2b - Start the local http listener which will receive data from Telemetry API
  console.log("[index:main] Starting the xray listener");
  const xrayTraceListener = xrayListener.start();
  console.log("[index:main] xray listener started at", listenerUri);
  ...
  const waitForArrayToBeNotEmpty = async (array) => {
  ...
  while (true) {
    ...
    console.log("[index:main] Next event", event);
    switch (event.eventType) {
      case "INVOKE":
        ...
        // });
        await waitForArrayToBeNotEmpty(telemetryListener.eventsQueue);
        console.log(
          "[index:main] Telemetry listener events",
          telemetryListener.eventsQueue.length
        );
        // telemetryListener.eventsQueue = [];
        await telemetryDispatcher.dispatch(
          telemetryListener.eventsQueue,
          false
        );

        await xrayDispatcher.dispatch(xrayListener.eventsQueue, false);
        break;
      case "SHUTDOWN":
